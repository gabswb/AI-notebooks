...
def astar_planning(start, goal, actions, grid_limits,
    ...
    while True:
        ...
        for i, action in enumerate(actions): 
            ...
            next_pos = (cur_node.pos[0] + action[0], cur_node.pos[1] + action[1])
            next_pos = np.array([cur_node.pos[0] + action[0], cur_node.pos[1] + action[1]])

            next_idx = get_grid_index(next_pos, grid_limits)
            if not is_valid(next_pos, grid_limits, costmap, robot_size, threshold) or next_idx in closedset:
                continue

            if next_idx not in openset:
                node = Node(next_pos, next_idx, cur_node.cost + costmap[next_idx], np.linalg.norm(np.array(next_pos)-goal), cur_idx)
                openset[next_idx] = node

            elif (cur_node.cost + costmap[next_idx]) < openset[next_idx].cost:
                    openset[next_idx].cost = cur_node.cost + costmap[next_idx]
                    openset[next_idx].prev_idx = cur_idx
            #------------------------------------------------------------
    ...
    # while ....
    #     path.append(...)      
    print("len=", len(CLOSED_SET))
    cur_idx = goal_node.idx
    while cur_idx != start_node.idx:
        node = CLOSED_SET[cur_idx]
        path.append(node.pos.tolist())
        cur_idx = node.prev_idx
    # node = goal_node
    # print(node)
    # while node.prev_idx != start_node.idx:
    #     path.append(node.pos.tolist())
    #     node = CLOSED_SET[node.prev_idx]
    #     print(node  )
